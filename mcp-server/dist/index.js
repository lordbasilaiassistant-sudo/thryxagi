...
import { LAUNCHPAD_ABI, PLATFORM_ROUTER_ABI, CREATOR_TOKEN_ABI, ERC20_ABI, } from "./contracts/abis.js";
...
server.tool("get_platform_stats", "Get OBSD LaunchPad platform statistics — total launches, ETH fees collected, contract addresses", {}, async () => {
...
// --- Tool: list_launches ---
server.tool("list_launches", "List tokens launched on the OBSD LaunchPad. Optionally filter by creator address.", {
    ...
    let indices;
    if (creator) {
        const ids = await launchPad.getCreatorLaunches(creator);
...
// --- Tool: get_token_info ---
server.tool("get_token_info", "Get detailed info about a token deployed on the OBSD LaunchPad — stats, pool, creator, earnings, fees", {
    address: z.string().describe("Token contract address"),
}, async ({ address }) => {
    const token = new ethers.Contract(address, CREATOR_TOKEN_ABI, provider);
    const [name, symbol, totalSupply, creator, pool, totalBurned, totalOBSDToCreator, totalOBSDToTreasury, pendingFees, totalFeeBps, burnFeeBps,] = await Promise.all([
        token.name(),
        token.symbol(),
        token.totalSupply(),
        token.creator(),
...
server.tool("get_creator_earnings", "Check OBSD earnings for a creator address across all their launched tokens", {
    address: z.string().describe("Creator payout address"),
}, async ({ address }) => {
    const ids = await launchPad.getCreatorLaunches(address);
    if (ids.length === 0) {
        return {
            content: [
                {
    ...
    const earnings = await Promise.all(ids.map(async (id) => {
    ...
    const totalEarned = earnings.reduce((sum, e) => sum + parseFloat(e.obsdEarned), 0);
    return {
        content: [
            {
                type: "text",
...
// --- Tool: quote_buy ---
server.tool("quote_buy", "Get a quote for buying a token with ETH via the PlatformRouter (includes 0.5% ETH fee)", {
...
// --- Tool: launch_token ---
server.tool("launch_token", "Deploy a new token on the OBSD LaunchPad. Requires DEPLOYER_PRIVATE_KEY env var. The token is paired with OBSD on Aerodrome — the creator earns OBSD on every swap forever.", {
